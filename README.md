...
- 수작업을 배제하고 단 몇 초 만에 처리되는 자동 파싱부터 ML 검증, 시각화에 이르는 엔드투엔드 노트북 파이프라인을 구축하여 향후 진행될 다양한 각도에서의 실험에서도 별도의 수정 없이 재현 가능한 환경을 마련

- **대규모 연산 최적화**: Brute‑force $O(N^2)$ 루프(11,164 원자)를 cKDTree 기반 $O(N\log N)$ 알고리즘으로 교체해 실행 시간을 **≈ 0.8 s** → **≈ 0.001–0.002 s** 로 감소
